...
## Directories
ROOT=/fast/users/a1649239/GeneAnalysis/liverNasa
...
NumberOfUniqueBioSamples=$(ls ${TRIMDATA}/fastq/*trim1*fastq.gz | sed 's!.*/!!'  | sed 's/_.*_trim1.fastq.gz//' | uniq )

echo " Unique biological samples detected ${NumberOfUniqueBioSamples}"

for uniqueSamples in $NumberOfUniqueBioSamples
do
echo "Merging lanes for bioloigcal Sample: $uniqueSamples"

cat ${TRIMDATA}/fastq/${uniqueSamples}* > ${TRIMDATA}/fastqMerged/${uniqueSamples}_R1_trim1.fastq.gz

echo "Done Merging files to ${TRIMDATA}/fastqMerged/${uniqueSamples}_R1_trim1.fastq.gz"
done


fastqc -t ${CORES} -o ${TRIMDATA}/FastQC --noextract ${TRIMDATA}/fastqMerged/*.fastq.gz
